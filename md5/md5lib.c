...
char *MDString(char *string, unsigned int ilen)
{
	MD5_CTX context;
	unsigned char digest[16];
	char output1[32];
	static char output[33] = {""};
	unsigned int len = ilen;//strlen (string);
	int i;
	MD5Init(&context);
...
char *MDFile_key(char *pFileName, char *pKey)
{
	...
	if((file = fopen(pFileName, "rb")) == 0) {
		printf("%s can't be openedn", pFileName);
		return 0;
	} else {
		...
		if(key_len > 64) {
			...
			MD5Init(&tctx);
			MD5Update(&tctx, (unsigned char *) pKey, key_len);
			MD5Final(tk, &tctx);
...
char *MDFile_key_len(char *pFileName, char *pKey, unsigned int uiLen)
{
	...
	if((file = fopen(pFileName, "rb")) == 0) {
		...
		*/
		...
		while(uiLen) {
			len = fread(buffer, 1, uiLen > 1024 ? 1024 : uiLen, file);
