...
unsigned RTSPClient::sendRequest(RequestRecord* request) {
  ...
  do {
    ...
    if (!fRequestsAwaitingConnection.isEmpty())//第一次进来的时候此处不成立
	{
      connectionIsPending = True;
    }
	else if (fInputSocketNum < 0)
	{ // we need to open a connection
      int connectResult = openConnection();//第一次进来入到此处
      if (connectResult < 0)
	...
	{
      fRequestsAwaitingConnection.enqueue(request);//把下一步压入队列
      return request->cseq();
    }
	...
	{
		//printf("\033[34m""setRequestFields is not ture""\033[0m");
      	break;
    }
	else
	{
		printf("\033[34m""setRequestFields is ture""\033[0m");
	}

    char const* contentStr = request->contentStr(); // by default
    printf("\033[34m""the contentStr is %s\n""\033[0m", contentStr);
	...
	printf("\033[32m""authenticatorStr =%s fBaseURL=%s \n""\033[0m", authenticatorStr, fBaseURL);
	...
	{
		printf("\033[32m""\n/********************************/\n""\033[0m");
		printf("\033[35m""we send the cmd =%s \n""\033[0m", cmd);
		printf("\033[32m""\n/********************************/\n""\033[0m");
	}
	...
	{
	printf("the fTunnelOverHTTPPortNum = %d request->commandName()=%s\n",fTunnelOverHTTPPortNum, request->commandName());
...
Boolean RTSPClient::setRequestFields(RequestRecord* request,
  ...
  if (strcmp(request->commandName(), "DESCRIBE") == 0)
  {
	...
	printf("setRequestFields desicer\n");
  }
...
char* RTSPClient::createAuthenticatorString(char const* cmd, char const* url) {
  ...
  {
  	printf("\033[32m"" auth.username=%s!!\n""\033[0m", auth.username());
	...
	{ // Basic authentication
		printf("\033[34m"" Basic authentication!!\n""\033[0m");
...
{
	printf("cseq=%d commandName=%s session=%p\n", cseq, commandName, session);
}
