  ...
  try {
    ...
    if (thisNALUnitEndsAccessUnit) {
      ...
      ++usingSource()->fPictureCount;
      	...
      	m_frameRate = usingSource()->fFrameRate;
		gettimeofday(&m_nexttime, NULL);
#if 0

		int sectime = 2;
		if(m_nexttime.tv_sec - m_FirstTime.tv_sec < sectime)
		{
			m_frameCount++;
		...
		{
			int lost_num = 2;
			m_lost += m_frameRate * sectime - m_frameCount;
			if(m_lost > lost_num || m_lost < -lost_num)
			{
				usingSource()->fPresentationTime = m_nexttime;

				m_lost = 0;

				printf("\033[32m""=========%d m_frameRate:%d %p===========\n""\033[0m", m_frameCount, m_frameRate, this);
			}
			m_FirstTime = m_nexttime;
			//printf("\033[33m""****** %d m_frameRate:%d %p*********\n""\033[0m", m_frameCount, m_frameRate, this);
			m_frameCount = 1;
		}
#else
		int nStreamTime = 1000/m_frameRate;
		int nErrorTime = (usingSource()->fPresentationTime.tv_sec - m_nexttime.tv_sec) * 1000 +
				(usingSource()->fPresentationTime.tv_usec - m_nexttime.tv_usec)/1000;
		if(nErrorTime > nStreamTime * 7 || nErrorTime < -(nStreamTime * 7))
		{
			printf("\033[32m""=========nErrorTime:%d m_frameRate:%d %p===========\n""\033[0m", nErrorTime, m_frameRate, this);
			usingSource()->fPresentationTime = m_nexttime;
		}

#endif      // Note that the presentation time for the next NAL unit will be different:
