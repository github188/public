...
MediaSession* MediaSession::createNew(UsageEnvironment& env,
				      char const* sdpDescription, int turnAudio) {
  MediaSession* newSession = new MediaSession(env);
  if (newSession != NULL) {
    if (!newSession->initializeWithSDP(sdpDescription, turnAudio)) {
      delete newSession;
      return NULL;
    }
  }
...
Boolean MediaSession::initializeWithSDP(char const* sdpDescription, int turnAudio) {
  ...
  while (sdpLine != NULL) {
    ...
    const char *addFind = strstr(sdpLine, "a=framesize");
    int add = 0;
    if(addFind)
    {
        sscanf(addFind, "a=framesize:%d %d-%d",
        		&add, &subsession->width, &subsession->height);
        printf("**********\n width:%d height:%d \n****************\n", subsession->width, subsession->height);
    }
...
Boolean MediaSession::parseSDPLine_s(char const* sdpLine) {
  ...
  if (sscanf(sdpLine, "s=%[^\r\n]", buffer) == 1) {
	...
	printf("parseSDPLine_s fSessionName=%s\n", fSessionName);
    parseSuccess = True;
  }
  delete[] buffer;
